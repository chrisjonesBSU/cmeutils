...
def get_excluded_pairs(bond_graph, excluded_bond_depth):
    ...
    #TODO: Vecotrize this instead of doing 2 for loops?
    # Not a big deal if bonds aren't changing.
    excluded_pairs = set()
    for i in bond_graph.nodes:
        ...
        for j, dist in lengths.items():
            if j > i and dist <= excluded_bond_depth:  # j > i avoids storing both (i,j) and (j,i)
...
def filter_nlist(nlist, excluded_pairs_encoded, query_indices, point_indices, max_idx):
    ...
    # The indices in excluded_pairs are the global particle indices (no filtering)
    ...
    # np.isin does a vectorized set lookup — no Python loop
